...
void Merge(vector<int> &vec, int front, int mid, int end) {
	...
	vector<int> left(vec.begin()+front, vec.begin()+mid+1); //left subarray
	vector<int> right(vec.begin()+mid+1, vec.begin()+end+1); //right subarray
	
	int idxLeft = 0, idxRight = 0; // index of left subarrray and right subarray
	
	// insert limit value to the end 
...
// write to file with vector
void WriteToFile(vector<int> &vec, string fileName)
{
    ofstream outputfile;
    outputfile.open (fileName.c_str());
...
//  MergeSort(arrB, 0, N - 1);
    MergeSort(vecB, 0, N - 1);
//  MergeSort(data, 0, N - 1);
    printf("Time taken: %.3fs\n", (double)(clock() - tStart) / CLOCKS_PER_SEC);
//  PrintArray(arrB, N);
    PrintArray(vecB);
    WriteToFile(vecB, "outputB.txt");
...
//  qsort(data, N, sizeof(int), comparetor );
    printf("Time taken: %.3fs\n", (double)(clock() - tStart) / CLOCKS_PER_SEC);
    PrintArray(arrD, N);
